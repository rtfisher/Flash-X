		    ...
		    int* existing)
{
  ...
  *(local_filename+len) = '\0';
  ...
  if (io_es_id != 0){
    ...
    hbool_t op_failed;
    H5ESwait(io_es_id, H5ES_WAIT_FOREVER, &num_in_progress, &op_failed);
    wtime1 = MPI_Wtime();
    fprintf(stderr, "H5ESwait took %.2f seconds\n", wtime1-wtime0);
  }
#endif
  ...
  */ierr = H5Pset_fapl_mpio(acc_template, io_comm, FILE_INFO_TEMPLATE);
  ...
  if(*existing == 0) {
    *file_identifier = H5Fcreate_async(local_filename, H5F_ACC_TRUNC, 
				 H5P_DEFAULT, acc_template, io_es_id);
  } else {
    *file_identifier = H5Fopen_async(local_filename, H5F_ACC_RDWR, acc_template,io_es_id);
  }
#else
  if(*existing == 0) {
...
#ifdef FLASH_IO_ASYNC_HDF5
  *file_identifier = H5Fopen_async(local_filename,H5F_ACC_RDONLY,acc_template,io_es_id);
#else
...
void FTOC(io_h5close_file)(hid_t* file_identifier)
{
  ...
  /* close the file */
#ifdef FLASH_IO_ASYNC_HDF5
  H5Fclose_async(*file_identifier,io_es_id);
#else
...
void FTOC(io_h5_close_async)()
{
   /* printf( "calling close async..\n"); */
#ifdef FLASH_IO_ASYNC_HDF5
   size_t num_in_progress;
   hbool_t op_failed;
   if (io_es_id != 0) {
      ...
      if (num_in_progress != 0) 
         printf("After H5ESwait, still has async operations in progress!");
      H5ESclose(io_es_id);
   }
#endif 
}
