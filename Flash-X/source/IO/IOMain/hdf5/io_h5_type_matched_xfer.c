			...
			const int dims, const int numFileBlks[], void * pData)
{
  ...
  /* useExtraFloatBuffer is not necessary since HDF5 can do type conversion 
   * automatically and supports larger than INT_MAX elements*/
  ...
  err = io_h5_xfer(myPE, hFileID, xferType, hXferList, datasetName,
		   ...
		   hDiskStart, hDiskCount, dims, numFileBlks, pHdf5Buf);
  assert(err == 0);
